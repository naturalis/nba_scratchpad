...
function storeAvailableOperators( operators )
{
	dynamicOperatorList=dynamicOperatorList.concat( operators ).filter( arrayFilterOnlyUnique );
}

function setDynamicOperatorList()
{
	if (dynamicOperatorList.length>0)
	{
		queryOperators.length=0
		queryOperators=dynamicOperatorList;
		setMessage( "loaded dynamic operator list" );
	}
	else
	{
		setMessage( "loaded default operator list (couldn't read dynamic)" );
	}
}
...
function processFieldData( docType, data )
{
	...
	for(var field in data)
	{
		...
		storeAvailableOperators( f.allowedOperators );
	}
